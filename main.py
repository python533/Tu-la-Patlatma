...
def update():
    ...
    if ball.y < -5:
        message = Text(text='Kaybettin', scale=2, origin=(0, 0), background=True, color=color.blue)
        application.pause()
    if len(bricks) == 0:
        message = Text(text='Kazandın', scale=2, origin=(0, 0), background=True, color=color.blue)
        application.pause()
